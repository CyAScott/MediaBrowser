...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260306214911_ExpandPasswordLength') THEN

    ALTER TABLE `users` MODIFY COLUMN `password_hash` varchar(125) CHARACTER SET utf8mb4 NOT NULL;
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260306214911_ExpandPasswordLength', '9.0.9');
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20260316031943_Chapters') THEN

    ALTER TABLE `media` ADD `parent_id` char(36) COLLATE ascii_general_ci NULL;
...
BEGIN
    ...
    ALTER TABLE `media` ADD `start` double NULL;
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20260316031943_Chapters', '9.0.9');
